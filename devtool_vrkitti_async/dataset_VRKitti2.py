...
class VirtualKITTI2(data.Dataset):
    def __init__(self, args, root, entries):
    ...
    def __getitem__(self, index):
        try:
            ...
            raise Exception("Index %d is broken, load next index" % index)
        ...
        data_blob = self.wrapup(img1, img2, flowmap, depthmap, intrinsic, renamed_ins, renamed_poses, renamed_ang, renamed_scale, tag)
        return data_blob

    def __len__(self):
        return len(self.entries)

    def wrapup(self, img1, img2, flowmap, depthmap, intrinsic, renamed_ins, renamed_poses, renamed_ang, renamed_scale, tag):
    ...
    def read_imgpair_flow_depth_instance_intrinsic(self, index):
        img1 = np.array(Image.open(self.rgb1_paths[index])).astype(np.uint8)
        img2 = np.array(Image.open(self.rgb2_paths[index])).astype(np.uint8)
